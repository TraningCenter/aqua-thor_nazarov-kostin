    ...
    public List<RelativeCell> getCellsInRange(CellGrid cellGrid, Vector position, Integer range) {
        ...
        for (int relativeY = -range; relativeY <= range; relativeY++)
            for (int relativeX = -range; relativeX <= range; relativeX++) {
                ...
                if (positionX < 0)
                    positionX = cellGrid.getSize().getX() + positionX;
                else if (positionX >= cellGrid.getSize().getX())
                    positionX = positionX % cellGrid.getSize().getX();

                if (positionY < 0)
                    positionY = cellGrid.getSize().getY() + positionY;
                else if (positionY >= cellGrid.getSize().getY())
                    positionY = positionY % cellGrid.getSize().getY();
